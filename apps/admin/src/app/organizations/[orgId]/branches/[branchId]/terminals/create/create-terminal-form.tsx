...
export function CreateTerminalForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!code?.trim()) validationErrors.push({ field: 'code', message: 'Terminal code is required' });
    if (!name?.trim()) validationErrors.push({ field: 'name', message: 'Terminal name is required' });
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      {success && <p role="status">Terminal created successfully</p>}
    </form>
  );
}
