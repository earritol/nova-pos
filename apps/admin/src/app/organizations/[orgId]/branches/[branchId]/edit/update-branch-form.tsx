...
export function UpdateBranchForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!input.name?.trim()) validationErrors.push({ field: 'name', message: 'Branch name cannot be empty' });
    if (!input.address?.trim()) validationErrors.push({ field: 'address', message: 'Address cannot be empty' });
    if (!input.phone?.trim()) validationErrors.push({ field: 'phone', message: 'Phone cannot be empty' });

    if (validationErrors.length > 0) { setErrors(validationErrors); return; }

    // TODO: Call OrganizationApplicationService.updateBranch()
    setSuccess(true);
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      {success && <p role="status">Branch updated successfully</p>}
    </form>
  );
}
